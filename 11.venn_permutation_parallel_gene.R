# permutation test:overlap genes between parallel DEG and genes based on parallel DEU####
...
temp=c(parallel_DEG_genes$x,parallel_DEU_genes$x)
temp.tbl=as.data.frame(table(temp))
commongenes=subset(temp.tbl, Freq>=min_overlap);print(commongenes)
N_obs=length(commongenes$temp);print(N_obs)#44
write.csv(commongenes,"./overlapgenes.csv",row.names = F)
...
for (N in 1:iter){
  ...
  permset1=sample(merged_all_DEG_genes$x,length(parallel_DEG_genes$x)) 
  # randomly sample as many as candidate genes in population 2
  permset2=sample(merged_all_DEU_exons$x,length(parallel_DEU$x)) 
...
p=N_perm/N
print(paste0("Permutation test p-value = ",p))# Permutation test p-value = 0 #
...
### plot a histogram/density showing the significance, p=0
...
ggsave("E:/liftoff/permutation_DEandDS_overlap.png", device="png",
       #path = "xxx",
...
library(VennDiagram)
setwd("E:/liftoff")
list2=list(parallel_DEG_genes$x,parallel_DEU_genes$x)
names(list2) <- c("parallelly expressed genes","parallelly spliced genes")

setwd("E:/paper_plot")
venn = venn.diagram(
  list2,
  filename = "venn_parallel_DE_DS.tiff",
  fontface = "bold",
  fontfamily = "Arial",
  ...
  cat.dist = c(0.05, 0.05), # 标签距离圆圈的距离
