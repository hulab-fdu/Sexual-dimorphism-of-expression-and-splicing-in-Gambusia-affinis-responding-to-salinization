...
dds_count=DESeqDataSetFromMatrix(countData=count_table_female, colData=sampleTable, design = ~salinity)
rlog_count=rlogTransformation(dds_count,blind=F)
head(assay(rlog_count),3)
rlog_count_data=as.data.frame(assay(rlog_count))
#-----------------#
...
disableWGCNAThreads()
datExpr_count= as.data.frame(t(rlog_count_data))#row:sample,col:gene
...
traitColors = numbers2colors(as.numeric(sampleTable$salinity), signed = FALSE) 
...
length(moduleLabels[moduleLabels == 0])#487
...
png("WGCNA_female_heatmap.sig1.png", width = 3000, height = 4000, res = 300)
par(mar = c(6, 15, 3, 3));
...
# Combine the genes of female WGCNA
...
write.table(combined_data_wgcna_female, "combined_wgcna_female.txt", col.names = FALSE, row.names = FALSE, sep = "\t")
...
sampleTable$salinity <- factor(sampleTable$salinity)
dds_count=DESeqDataSetFromMatrix(countData=count_table_male, colData=sampleTable, design = ~salinity)
rlog_count=rlogTransformation(dds_count,blind=F)
head(assay(rlog_count),3)
rlog_count_data=as.data.frame(assay(rlog_count))
#-----------------#
...
moduleColors=labels2colors(net$colors)
## number of sites without module (assigned to grey module):
length(moduleLabels[moduleLabels == 0])#386
# get genes per module #table(net$colors)
ExonsperModule <- NULL
for (c in seq(1, length(levels(as.factor(moduleColors))), by = 1)) {
  color <- levels(as.factor(moduleColors))[c]
...
# Display the correlation values within a heatmap plot
png("WGCNA_male_heatmap.sig1.png", width = 3000, height = 4000, res = 300)
par(mar = c(6, 15, 3, 3));
labeledHeatmap(Matrix = Sig.moduleTraitCor,
...
# Combine the genes of male WGCNA
...
write.table(combined_data_wgcna_male, "combined_wgcna_male.txt", col.names = FALSE, row.names = FALSE, sep = "\t")
